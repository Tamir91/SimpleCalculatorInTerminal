...
{	
	...
	printf("\n\tWelcome to simple calculator 1.0!\n");
	...
	while ((c = getchar()) && c != '2')
		...
		{	
			case '1':
				printf("Write expression: x # y, instead #: +, -, * or / \nand press \"unter\". ");				
				c = getchar();			
			if( c == '\n')		
				calculate1();
			printHelp();
			...
			default:
				printf("incorrect input! Press 1 or 2!\n");
			break;
		}
...
int calculate1(void)
{
	...
	enum oprtr {SUM, DIF, MULT, DIV};							/* Operators of expression. */
		...
		if(changeSign == -1)							/* Changing sign for second number.*/
			secN *= changeSign;
		if(posPoint > 0)
			secN = secN / (myPow1(10, posPoint));		/* Insert point to his place for second number. */
...
int myPow1(int base, int num)
{
	...
	/* This method print help information on screen. */
void printHelp(void)
{
	printf("\n\tFor calculate something press - \"1\".\n");
